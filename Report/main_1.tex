...
\node[node, right=10mm of llm] (score) {Scoring \& logs\\Accuracy / F1\\SQL exec.\ accuracy\\Latency, cost, expl.\ score};
...
\begin{table}[t]
\centering
\caption{Core Spotify audio features (only the features used in our experiments are listed here due to space constraints) used in Task A. Ranges reflect Spotify API conventions.}
\label{tab:features}
...
Figure~\ref{fig:pipeline} shows the end-to-end pipeline used for both tasks. It is drawn directly in \LaTeX{} (TikZ), ensuring precision, reproducibility, and consistency with the evaluation harness.

% Figure moved to Introduction section for proper floating.
...
\begin{table*}[t]
    \centering
    \caption{Text-to-SQL Results }
    \label{tab:sql_results_wide}
    % Columns: l (left) for Model, c (center) for the 5 numeric columns
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model} & \textbf{Exact-Match} & \textbf{Execution Accuracy} & \textbf{Schema Errors} & \textbf{Syntax Errors} & \textbf{Semantic Errors} \\
        \midrule
        Gemini & 32 & 40 & 0 & 0 & 0 \\
        GPT    & 11 & 32 & 0 & 0 & 8 \\
        Claude & 15 & 31 & 0 & 0 & 8 \\
