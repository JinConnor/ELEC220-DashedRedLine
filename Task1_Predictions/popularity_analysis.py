...
for model, path in MODEL_FILES.items():
    ...
    merged_data = pd.merge(prediction_data, actual_pop_data, on="track_id", suffixes=("_pred", "_true"))
    
    if "track_id" in merged_data.columns:
        merged_data = pd.merge(orig_song_data, merged_data, on=["track_id"])
    ...
    print(f"{model:<10} | {str(acc)[:6]:<6} | {str(f1)[:6]:<6} | {str(mae)[:6]:<6} | {str(hall_rate)[:6]}")
